...
start = (0, 2) #start point copied from the manual
goal = (4, 4) #gaol " " " "

def bfs(graph, start, goal):
    directions = [(1, 0), (-1, 0), (0, 1), (0, -1)] #possible directions where the robot can go
    stack = [(start, [start])] #storing the visited square and path to reach to that square/position
    visited = set() #initialize a set

    while stack: #while the que is not empty
        (x, y), path = stack.pop() #retreive elements form the Q
        ...
        for dx, dy in directions: #traverst through all allowed direction
            ...
            if 0 <= nx < len(graph) and 0 <= ny < len(graph[0]) and graph[nx][ny] == 1 and (nx, ny) not in visited: #check is it possible to go that direction
                stack.append(((nx, ny), path + [(nx, ny)])) #append to the stack with the path.
